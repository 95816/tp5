...
php;
    ...
    public function tagWebuploadercss(){
        $str=<<<php
<link rel="stylesheet" href="__STATIC__/webuploader/xb-webuploader.css">
...
<script src="__STATIC__/webuploader/webuploader.js"></script>
php;
        return $str;
    }
    ...
    public function tagWebuploader($tag){
        $url=isset($tag['url'])?$tag['url']:'/api.php/Upload/ajax_upload';
        $name=isset($tag['name'])?$tag['name']:'file_name';
        $word=isset($tag['word'])?$tag['word']:'或将照片拖到这里，单次最多可选300张';
        $is_multiple =isset($tag['is_multiple'])?$tag['is_multiple']:'false';
...
jQuery(function() {
    ...
    function addFile( file ) {
        var \$li = \$( '<li id="' + file.id + '">' +
                ...
                '<input class="zhaobo-filename" type="hidden" name="{$name}[]">'+
                '</li>' ),
    ...
    uploader.onUploadSuccess=function(file ,response){
        \$('#'+file.id +' .zhaobo-filename').val(response.name)
    }
