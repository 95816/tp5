...
class Uploader extends TagLib
{
    protected $tags = array(
        ...
        'webuploader'=>array('attr'=>'name,url,word,is_multiple','close'=>0),
    );

    public function tagJquery()
    {
        $str=<<<php
<script src="__STATIC__/webuploader/js/jquery.js"></script>
php;
        return $str;
    }
    ...
    {
        $parseStr=<<<php
<link rel="stylesheet" href="__STATIC__/webuploader/css/xb-webuploader.css">
...
php;
    ...
    public function tagWebuploaderjs()
    {
        $parseStr=<<<php
<script>
    var BASE_URL = '__STATIC__/webuploader';
</script>
<script src="__STATIC__/webuploader/js/dist/webuploader.js"></script>
php;
        return $parseStr;
    }
    public function tagWebuploader($tags)
    {
        $url = isset($tags['url'])?$tags['url']:'/api.php/Upload/ajax_upload';
        $name = isset($tags['name'])?$tags['name']:'file_name';
        $word = isset($tags['word'])?$tags['word']:'或将照片拖到这里，单次最多可选300张';
        $is_multiple = isset($tags['is_multiple'])?$tags['is_multiple']:false;
